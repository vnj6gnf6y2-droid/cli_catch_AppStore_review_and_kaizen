...
def _make_fixed_run() -> RunOrm:
    ...
    run.started_at = datetime(2026, 6, 3, 11, 58, 0, tzinfo=timezone.utc)
    run.finished_at = datetime(2026, 6, 3, 12, 0, 0, tzinfo=timezone.utc)
    run.status = "success"
...
def _make_fixed_reviews() -> list:
    ...
    for i in range(10):
        ...
        r.created_at = datetime(2026, 5, 28 + (i % 3), tzinfo=timezone.utc)
        r.updated_at = None
